...
@endif
        ...
        <table class="min-w-full table-auto">
          <thead>
            <tr class="bg-gray-200 text-gray-600 uppercase text-sm leading-normal">
              ...
              <th class="py-3 px-6 text-center">Stok Masuk</th>
              <th class="py-3 px-6 text-center">Stok Keluar</th>
    ...
    <tr class="border-b border-gray-200 hover:bg-gray-100">
        ...
        <td class="py-3 px-6 text-center">{{ $item->stok_masuk }}</td>
        <td class="py-3 px-6 text-center">{{ $item->stok_keluar }}</td>
        <td class="py-3 px-6 text-left">
          <div class="flex space-x-2">
              <!-- Form Edit -->
              <form action="{{ url('/admin_dashboard/screen_opname/edit/' . $item->id) }}" method="GET">
                  <button type="submit" 
                          class="bg-orange-500 text-white px-5 py-2 rounded-lg hover:bg-orange-600">
                      Edit Product
                  </button>
              </form>
      
              <!-- Form Delete -->
              <form action="{{ route('screenopname.delete', $item->id) }}" method="POST" 
          ...
          <div class="mt-3 flex space-x-5">
            ...
            <form action="{{ url('/admin_dashboard/screen_opname/tambah_stok/' . $item->id) }}" method="GET">
                <button type="submit" 
                        class="bg-green-500 text-white px-5 py-2 rounded-lg hover:bg-green-600">
                    Add Stock
                </button>
            </form>
